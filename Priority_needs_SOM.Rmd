...
```{r pressure, echo=FALSE}
#Cleaning up the raw dataframe and subsetting UNHCR dataframe
UNHCR <- UNHCR_raw %>% 
  ...
  filter(priority_need == "Food")  #just showing Food as priority need
...
png(
  width = 8,
  height = 6,
  file = here("figures/circosplots.png"),
  units = "in",
  res = 600,
  family = "Myriad Pro"
)
...
for(i in c("all","Drought related","Conflict/Insecurity", "Food")){
  # i = "all"
  ...
  title(
    main = switch(i,
                  all = "Priority Needs Total IDPs",
                  `Drought related` = "Priority Needs Drought IDPs",
                  `Conflict/Insecurity` = "Priority Needs Conflict IDPs"),
    cex.main = 1,
    family = "Myriad Pro", 
